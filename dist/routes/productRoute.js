...
router.get("/", async (req, res) => {
  ...
  const sortOrder = req.query.sortOrder ? req.query.sortOrder === 'highest' ? {
    price: 1
  } : {
    price: -1
  } : {
...
router.post('/:id/reviews', _util.isAuth, async (req, res) => {
  ...
  if (product) {
    ...
    product.rating = (product.reviews.reduce((a, c) => c.rating + a, 0) / product.reviews.length).toFixed(2);
...
router.delete("/:id", _util.isAuth, _util.isAdmin, async (req, res) => {
  ...
  if (deletedProduct) {
    ...
    res.send("Error..Deletion unsucessful.");
  }
});
